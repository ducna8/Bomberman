...
public class Brick extends immobileEntity {
  ...
  public Image explodingBrick() {
    if (img == explodedBrick.get(0)) return explodedBrick.get(1);
    if (img == explodedBrick.get(1)) return explodedBrick.get(2);
    if (img == explodedBrick.get(2)) return null;
    if (img == null) return null;
    else return explodedBrick.get(0);
  }
  @Override
  public void update() {
    if(difference == true){
      for(int i =0; i< CreateMap.)
    }
  }
}
